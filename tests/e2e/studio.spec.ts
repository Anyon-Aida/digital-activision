...
for (const locale of ["hu", "en"] as const) {
  test(`${locale.toUpperCase()} Studio route exposes the V3 editorial service topology`, async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-status='needs-owner-confirmation']"),
    ).toHaveCount(0);
    await expect(page.locator("[data-preview-only-price]")).toHaveCount(0);
